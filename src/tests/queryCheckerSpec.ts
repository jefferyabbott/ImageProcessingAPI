...
it('expect warning if filename not provided', () => {
    ...
    const height = 200
    const width = 200
    expect(validateQueryString(filename as string, height, width)[0]).toEqual('filename is required');
})
...
it('expect warning if height not provided', () => {
    ...
    let height: unknown
    const width = 200
    expect(validateQueryString(filename, height, width)[0]).toEqual('height is required');
})
...
it('expect warning if height is negative', () => {
    ...
    const height = -1
    const width = 200
    expect(validateQueryString(filename, height, width)[0]).toEqual('height must be greater than zero');
})
...
it('expect warning if width not provided', () => {
    ...
    const height = 200
    let width: unknown
    expect(validateQueryString(filename, height, width)[0]).toEqual('width is required');
})
...
it('expect warning if width is negative', () => {
    ...
    const height = 50
    const width = -100
    expect(validateQueryString(filename, height, width)[0]).toEqual('width must be greater than zero');
})
